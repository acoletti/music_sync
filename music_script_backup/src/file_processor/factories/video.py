...
class VideoFactory(FileFactory):
    """Factory interface for creating video info objects."""
    def __init__(self):
        super().__init__()
    ...
    def create_batch(self, paths: List[Path], fast: bool = False) -> List[VideoInfo]:
        """Create multiple video info objects."""
        return [
            video_info
            for path in paths
            if (video_info := self.create(path, fast=fast)) is not None
        ]
...
class DefaultVideoFactory(VideoFactory):
    """Default implementation of VideoFactory."""
    def create(self, path: Path, fast: bool = False) -> Optional[VideoInfo]:
        """Create a new video info object."""
        ...
        try:
            ...
            print(f"Error creating video info for {path}: {e}")
            return None 