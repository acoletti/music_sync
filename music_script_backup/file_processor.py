...
def is_audio_file(file_path: Path, extensions: Set[str]) -> bool:
...
class FileProcessorConfig:
    ...
    def update_stats(self, cleaned_count: int = 0) -> None:
        ...
        config.total_processed = len(self.processed_folders)
...
# Update AudioProcessor to inherit from AbstractProcessor
class AudioProcessor(AbstractProcessor):
    def __init__(self):
        self._audio_factory = DefaultAudioFactory()

    def process_file(self, file_path: Path) -> Optional[AudioInfo]:
        return self._audio_factory.create(file_path)

    def get_files(self, folder_path: Path) -> List[AudioInfo]:
        return [
    ...
    def _is_audio_file(self, file_path: Path) -> bool:
        return file_path.is_file() and file_path.suffix.lower() in {fmt.value for fmt in AudioFormat}

# Update DocumentProcessor to inherit from AbstractProcessor
...
# Update VideoProcessor to inherit from AbstractProcessor
...
def calculate_file_hash_safe(file_path: Path, chunk_size: int) -> Optional[str]:
    ...
    try:
        return calculate_file_hash(file_path, chunk_size)
    except Exception as e:
